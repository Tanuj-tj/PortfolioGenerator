...
def loginUser(request):
    ...
    # Restrict the user to access /login when then are already logged in
...
def registerUser(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # making sure that the username is lowercase
...
def userAccount(request):
    ...
    # Getting the one to one relationship
...
def editAccount(request):
    profile = request.user.profile
    form = ProfileForm(instance=profile)  # Prefill the data with user info
...
@login_required(login_url='login')
def inbox(request):
    # Get the current logdin profile
    profile = request.user.profile
